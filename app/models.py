...
These classes represent the tables in the database. SQLAlchemy
uses these to manage all interactions with PostgreSQL, so we can
write Python code instead of SQL.
"""

from sqlalchemy import (
    Column, Integer, String, Numeric, Date, Text, SmallInteger, ForeignKey, LargeBinary,
)
...
class Employee(Base):
    ...
    manager = relationship("Employee", remote_side=[employeeNumber], backref="subordinates")
...
class Payment(Base):
    ...
    customerNumber = Column(
        ...
        ForeignKey("customers.customerNumber"), primary_key=True,
    )
...
class Order(Base):
    ...
    customerNumber = Column(
        "customerNumber", Integer,
        ForeignKey("customers.customerNumber"), nullable=False,
    )
...
class OrderDetail(Base):
    ...
    orderNumber = Column(
        "orderNumber", Integer,
        ForeignKey("orders.orderNumber"), primary_key=True,
    )
    productCode = Column(
        "productCode", String(15),
        ForeignKey("products.productCode"), primary_key=True,
    )
